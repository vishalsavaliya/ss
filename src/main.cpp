...
unsigned int nStakeMinAge =  8 * nOneHour; // as zero time weight
unsigned int nStakeMaxAge = 2 * 24 * nOneHour; //  as full weight
...
const string strMessageMagic = "SKOIN Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SKOIN
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // SKOIN: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SKOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "skoin genesis";
        CTransaction txNew;
        txNew.nTime = 1508564467;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1508564467;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 523598; // !fTestNet ? 1575379 : 46534;
...
unsigned char pchMessageStart[4] = { 0xb9, 0xf9, 0xe4, 0xa5 };
