...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SKOINTEST2\r");
            Send(hSocket, "WHO #SKOINTEST2\r");
        } else {
            // randomly join #SKOIN00-#SKOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SKOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SKOIN%02d\r", channel_number).c_str());
        }
